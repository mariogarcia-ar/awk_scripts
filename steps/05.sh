...
if [ $run_step $comparison_operator 05 ]; then
    ...
    script="$SCRIPTS_DIR/awk/book/chap100/create_file.awk"
    ...
    script="$SCRIPTS_DIR/awk/book/chap100/backup.awk"
    ...
    script="$SCRIPTS_DIR/awk/book/chap100/get_file_info.awk"
    # Execute awk script with ',' as field separator
    awk -F, -f "$script" "$DATA_DIR/file_list.csv"
    ...
    script="$SCRIPTS_DIR/awk/book/chap100/md5.awk"
    # Execute awk script with ',' as field separator
    awk -F, -f "$script" "$DATA_DIR/file_list.csv"    
fi
